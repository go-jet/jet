...
func Query(ctx context.Context, db Queryable, query string, args []interface{}, destPtr interface{}) (rowsProcessed int64, err error) {
	...
	rowsProcessed, err = queryToSliceOrStruct(ctx, rows, destPtr)
...
// QueryPreparedStatement executes Query Result Mapping (QRM) of a prepared statement with a list of
// parametrized arguments `arg`, over database connection `db`.
// Destination can be either pointer to struct or pointer to slice of structs.
// If destination is pointer to struct a query result set is empty, method returns qrm.ErrNoRows.
...
// QueryRowsToSlice executes Query Result Mapping (QRM) over sql rows.
// Destination has to be pointer to slice.
// This method will not close rows upon completion.
