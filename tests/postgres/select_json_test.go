...
func TestSelectJson_InvalidDestination(t *testing.T) {
	t.Run("json obj", func(t *testing.T) {
		...
		testutils.AssertQueryJsonPanicErr(t, stmt, db, &[]model.Actor{}, "jet: SELECT_JSON_OBJ destination has to be a pointer to struct or pointer to map[string]any")
		testutils.AssertQueryJsonPanicErr(t, stmt, db, model.Actor{}, "jet: SELECT_JSON_OBJ destination has to be a pointer to struct or pointer to map[string]any")
	...
	t.Run("json arr", func(t *testing.T) {
		...
		testutils.AssertQueryJsonPanicErr(t, stmt, db, &model.Actor{}, "jet: SELECT_JSON_ARR destination has to be a pointer to slice of struct or pointer to []map[string]any")
		testutils.AssertQueryJsonPanicErr(t, stmt, db, []model.Actor{}, "jet: SELECT_JSON_ARR destination has to be a pointer to slice of struct or pointer to []map[string]any")
		testutils.AssertQueryJsonPanicErr(t, stmt, nil, &[]model.Actor{}, "jet: db is nil")
		testutils.AssertQueryJsonPanicErr(t, stmt, db, nil, "jet: destination is nil")
	})
